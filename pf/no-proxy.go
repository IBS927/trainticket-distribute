...
func setup_container(container_name string, service ServiceInfo, dnsOptionString string)(error){
		...
		*/
		command2 := fmt.Sprintf("sudo docker run --restart unless-stopped --cap-add=NET_RAW --network none %s --name %s -d kawanotatsuya/%s enp2s0f1",dnsOptionString, container_name, container_name)
