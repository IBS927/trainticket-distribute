...
func Proxy_Run_Handler(w http.ResponseWriter, r *http.Request) {
	// HTTP GETリクエスト
	...
	for container_name, service := range services_no_mysql {
		...
		*/
		...
		command4 := fmt.Sprintf("sudo docker run --network none %s --name %s_proxy -d kawanotatsuya/%s_proxy",dnsOptionString, container_name, container_name)
		...
		command12 := fmt.Sprintf("echo $(sudo docker inspect -f '{{.State.Pid}}' %s_proxy)", container_name)
...
func Proxy_Del_Handler(w http.ResponseWriter, r *http.Request) {
        // HTTP GETリクエスト
        resp, err := http.Get("http://localhost:8000/all")
        if err != nil {
                fmt.Println("Error fetching URL:", err)
		...
		command1 := fmt.Sprintf("sudo docker stop %s", container_name)
                ...
                command2 := fmt.Sprintf("sudo docker stop %s_proxy", container_name)
                output_2, err := session_and_command(command2, client)
                if err != nil {
                        fmt.Println("unable to create session or execute command:", err)
		...
		command4 := fmt.Sprintf("sudo docker rm %s_proxy", container_name)
                output_4, err := session_and_command(command4, client)
                if err != nil {
                        fmt.Println("unable to create session or execute command:", err)
