...
func AmbientHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Ambient Proxy 情報取得
	response, err := http.Get("http://localhost:8000/ambient_cluster")
        if err != nil {
                fmt.Println("Error fetching URL:", err)
                return
        }
        ...
        err = json.Unmarshal(body, &ambient_proxies)
        if err != nil {
                fmt.Println("Error parsing JSON:", err)
                return
        }
	...
	var ambient_proxy ProxyInfo
        for container_name, service := range services_no_mysql {
                ambient_proxy = ambient_proxies[i]
		i = (i + 1) % len(ambient_proxies)
...
func SnicAmbientHandler(w http.ResponseWriter, r *http.Request) {
        ...
        err = json.Unmarshal(body, &snic_proxies)
        if err != nil {
                fmt.Println("Error parsing JSON:", err)
                return
        }
	...
	var snic_proxy ProxyInfo
        for container_name, service := range services_no_mysql {
                snic_proxy = snic_proxies[i]
		i = (i + 1) % len(snic_proxies)
